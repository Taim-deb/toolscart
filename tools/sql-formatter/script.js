...
const loadFile = async () => {
  ...
  if (!f) return;
  try {
    $("#in").value = await T.readFile(f, [".sql", ".txt"]);
...
$("#process").onclick = () => {
  const kw = [
    ...
    "on", "and", "or", "limit", "offset"
  ];
  let out = $("#in").value.trim().replace(/\s+/g, " ");
  kw.sort((a, b) => b.length - a.length).forEach(k => {
    const r = new RegExp("\\b" + k.replace(/ /g, "\\s+") + "\\b", "ig");
    out = out.replace(r, m => "\n" + m.toUpperCase());
  });
  $("#out").value = out.replace(/^\n/, "").replace(/\n+/g, "\n").trim();
  T.setStatus("status", "Formatted.", "ok");
};

$("#copy").onclick = () => T.copy($("#out").value, "status");
$("#export").onclick = () => T.download("query.sql", $("#out").value || $("#in").value);
