...
const loadFile = async () => {
  ...
  if (!f) return;
  try {
    $("#in").value = await T.readFile(f, [".md", ".txt"]);
...
const update = () => $("#preview").innerHTML = T.md($("#in").value);

$("#in").addEventListener("input", update);
$("#process").onclick = update;
$("#copy").onclick = () => T.copy($("#in").value, "status");
$("#export").onclick = () => T.download("preview.md", $("#in").value);
