...
const loadFile = async () => {
  ...
  if (!f) return;
  try {
    $("#in").value = await T.readFile(f, [".txt", ".md", ".html", ".css", ".js", ".json", ".csv", ".sql"]);
...
$("#process").onclick = () => {
  try {
    $("#out").value = mode === "encode"
      ? btoa(unescape(encodeURIComponent($("#in").value)))
      : decodeURIComponent(escape(atob($("#in").value.replace(/\s+/g, ""))));
    T.setStatus("status", "Done.", "ok");
  } catch (e) {
    T.setStatus("status", "Invalid Base64.", "error");
  }
};

$("#copy").onclick = () => T.copy($("#out").value, "status");
$("#export").onclick = () => T.download("base64.txt", $("#out").value || $("#in").value);
